...
type ColumnDef<'row, 'value> =
  ...
  static member inline valueSetter (f:string -> 'value -> 'row -> unit) = columnDefProp<'row, 'value> ("valueSetter" ==> (fun x ->
...
type AgGrid() =
  ...
  static member inline className (v:string) = agGridProp<'row>("className" ==> v)
